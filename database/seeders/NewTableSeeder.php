...
class NewTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i = 1 ; $i <= 16 ; $i++){
            ...
            $mediaDepartmentCenter->image_url = "storage/media_center_departments/images/news/".$i.".png";
            $mediaDepartmentCenter->type = "new";
            $mediaDepartmentCenter->save();
            $mediaDepartmentCenter->translateOrNew("ar")->title =  "ألقى صاحب السمو الملكي الأمير تركي الفيصل كلمة بعنوان الحقيقة والعواقب في معهد بيكر للسياسات العامة";
            $mediaDepartmentCenter->translateOrNew("en")->title = "His Royal Highness Prince Turki Al Faisal delivered a speech entitled Truth and Consequences at the Baker Institute for Public Policy";
            $mediaDepartmentCenter->translateOrNew("ch")->title = "圖爾基費薩爾親王殿下在貝克公共政策研究所發表題為《真相與後果》的演講";
